...
const Ingredients = () => {
  ...
  const handleBatchesChange = useCallback((ingredientId, total) => {
    setIngredients((prev) =>
      prev.map((i) =>
        i.id === ingredientId ? { ...i, current_stock: total } : i
      )
    );
  }, []);
  ...
  const toggleIngredientStatus = async (ingredientId, currentStatus) => {
    try {
      ...
      const response = await fetchWithAuth(
        `${import.meta.env.VITE_INVENTORY_URL}/ingredients/${ingredientId}/`,
        {
          method: "PATCH",
      ...
      const normalized = {
        ...updated,
        current_stock: updated.current_stock ?? updated.total_stock ?? null,
      };
      setIngredients((prev) =>
        prev.map((i) =>
          i.id === ingredientId
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        default_unit_price:
          formData.default_unit_price === ""
            ? null
            : Number(formData.default_unit_price),
        restock_level:
          formData.restock_level === "" ? null : Number(formData.restock_level),
      ...
      const normalized = {
        ...saved,
        current_stock: saved.current_stock ?? saved.total_stock ?? null,
      };
      if (isEdit) {
        setIngredients((prev) =>
          prev.map((i) => (i.id === saved.id ? normalized : i))
        );
      } else {
  ...
  const displayIngredients =
    ingredients.length > 0 ? ingredients : sampleIngredients;
  ...
  const filteredIngredients = displayIngredients.filter((ingredient) => {
    ...
    if (expiryDateFrom || expiryDateTo) {
      const nextExpiry = ingredient.next_expiry_date;
      if (!nextExpiry) {
        // If ingredient has no expiry date, exclude it from expiry filter
        matchesExpiry = false;
      } else {
  ...
  const categories = [
    "All",
    ...Array.from(
      new Set(displayIngredients.map((i) => i.category).filter(Boolean))
    ),
  ];
  ...
  const lowStockAlerts = displayIngredients.filter((i) => {
    ...
    const restock =
      i.restock_level === "" ||
      i.restock_level === null ||
      i.restock_level === undefined
        ? null
        : Number(i.restock_level);
    if (stock === null || restock === null || Number.isNaN(restock))
      return false;
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              Track ingredients, stock levels, and batches
            </p>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-[#f08b51] mb-1">
                    {totalIngredients}
                  </p>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-gray-600">
              Manage your ingredient stock and batches
            </p>
          </div>
          ...
          <div className="px-6 py-4 border-b border-gray-200 space-y-4">
            <div className="flex flex-col gap-4">
              ...
              {/* Second row: Category and Expiry Date Range */}
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredIngredients.map((ingredient) => {
                  ...
                  const restock =
                    ingredient.restock_level === "" ||
                    ingredient.restock_level === null ||
                    ingredient.restock_level === undefined
                      ? null
                      : Number(ingredient.restock_level);
                  const low =
                    stock !== null &&
                    restock !== null &&
                    !Number.isNaN(restock) &&
                    stock <= restock;
                  return (
                    <tr key={ingredient.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {restock !== null && !Number.isNaN(restock)
                          ? `${restock}${
                              ingredient.unit_of_measurement
                                ? ` ${ingredient.unit_of_measurement}`
                                : ""
            ...
            <form onSubmit={submitForm} className="px-6 py-4 space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="px-4 py-2 rounded-md bg-[#f08b51] text-white hover:bg-[#d9734a]"
                >
                  {saving
                    ? "Saving..."
                    : editingIngredient
